...
func createPOSVendor(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		securityService := auth.NewSecurityService("default_secret")
		apiKey, _ := securityService.GenerateRandomString(32)
		apiSecret, _ := securityService.GenerateRandomString(64)
