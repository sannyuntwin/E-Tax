...
func (s *SecurityService) RequireRole(requiredRole string) gin.HandlerFunc {
	return func(c *gin.Context) {
...
func (s *SecurityService) GenerateToken(userID uint, username, role string) (string, error) {
	claims := jwt.MapClaims{
		"user_id":  userID,
		"username": username,
		"role":     role,
...
// Helper method to compare password with hash
func (s *SecurityService) comparePassword(password, hash string) bool {
	// Parse the hash, extract the salt, and derive the key from the password
	// Then compare with the provided hash in constant time
	salt, err := base64.RawStdEncoding.DecodeString(hash[:32])
	if err != nil {
		return false
	}
	
	hashBytes, err := base64.RawStdEncoding.DecodeString(hash[32:])
	if err != nil {
		return false
	}
	
	computedHash := argon2.IDKey([]byte(password), salt, s.passwordConfig.Iterations, s.passwordConfig.Memory, s.passwordConfig.Parallelism, s.passwordConfig.KeyLength)
	
	return subtle.ConstantTimeCompare(computedHash, hashBytes) == 1
}

// GenerateRandomString generates a random string of specified length
func (s *SecurityService) GenerateRandomString(length int) (string, error) {
	bytes := make([]byte, length)
	if _, err := rand.Read(bytes); err != nil {
		return "", err
	}
